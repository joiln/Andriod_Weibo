...
tls_config() {
	...
	while :; do
		...
		read -p "(例如：www.baidu.com): " domain
...
path_config() {
	echo
	while :; do
		echo -e "请输入想要 ${magenta}用来分流的路径$none , 例如 /myv2ray , 那么只需要输入 myv2ray 即可"
		read -p "$(echo -e "(默认: [${cyan}myv2ray$none]):")" path
		[[ -z $path ]] && path="myv2ray"
...
proxy_site_config() {
	echo
	while :; do
		echo -e "请输入 ${magenta}一个正确的$none ${cyan}网址$none 用来作为 ${cyan}网站的伪装$none , 例如 https://v2ray.com"
		echo -e "举例...你当前的域名是 $green$domain$none , 伪装的网址的是 https://v2ray.com"
		echo -e "然后打开你的域名时候...显示出来的内容就是来自 https://v2ray.com 的内容"
		echo -e "其实就是一个反代...明白就好..."
		echo -e "如果不能伪装成功...可以使用 v2ray config 修改伪装的网址"
		read -p "$(echo -e "(默认: [${cyan}https://v2ray.com$none]):")" proxy_site
...
shadowsocks_password_config() {
	...
	while :; do
		...
		read -p "$(echo -e "(默认密码: ${cyan}v2ray$none)"): " sspass
...
while :; do
	...
	echo
	echo "帮助说明: https://v2ray.com/"
	echo
	echo "搭建教程: https://toutyrater.github.io/"
	echo
	echo " 1. 安装"
	echo
	echo " 2. 卸载"
